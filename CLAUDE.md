...
## Structure
- `Meets/` is the application target, and holds only the entry point and the resources of the bundle.
- Everything else lives in local packages under `Modules/`, grouped by layer, `App` over `Features` over `Shared` over `Core`.
...
- Boundaries: the composition root depends on features, features depend on shared modules, shared modules depend on core modules. A feature never imports another feature, the coordinator is what joins them. Only value types cross a boundary, entities and events, never a view or a view model of another feature.
